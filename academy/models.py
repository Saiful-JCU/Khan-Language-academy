...
class Grammar(models.Model):
    day = models.ForeignKey(DayStrick, on_delete=models.CASCADE, limit_choices_to={"created_at": timezone.now().date()})
...
class GrammarRule(models.Model):
    grammar = models.ForeignKey(Grammar, on_delete=models.CASCADE, related_name = "rules", limit_choices_to={"day__created_at":timezone.now().date()})
...
class Vocabulary(models.Model):
    day = models.ForeignKey(DayStrick, on_delete=models.CASCADE, blank=True, null=True, limit_choices_to = {"created_at": timezone.now().date()})
...
class PracticeSentences(models.Model):
    day = models.ForeignKey(DayStrick, on_delete = models.CASCADE, limit_choices_to={"created_at": timezone.now().date()})
    word = models.ForeignKey(Vocabulary, on_delete=models.CASCADE, limit_choices_to={"day__created_at": timezone.now().date()})
